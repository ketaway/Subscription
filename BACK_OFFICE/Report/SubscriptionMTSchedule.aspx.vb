Public Class SubscriptionMTSchedule
    Inherits System.Web.UI.Page
    Private PrevDate, PrevShortcode, PrevShortcode_Topic, CurDate, CurShortcode, CurShortcode_Topic As String
    Private CountDupDate, CountDupShortcode, CountDupShortcode_Topic, rptCount As Integer
    ...
    Private Sub getReport()
        ...
        If (dtRange.Value <> "") Then
            ...
            rptReport.DataSource = objRPT.getContentSchedule(dlService.SelectedValue, dlTopic.SelectedValue, DateStart, DateEnd)
            rptReport.DataBind()
        End If
    ...
    Private Sub rptReport_ItemDataBound(sender As Object, e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles rptReport.ItemDataBound
        If (Not e.Item.DataItem("OPERATOR_A") Is DBNull.Value) Then
            e.Item.FindControl("chkAIS").Visible = True
        Else
            e.Item.FindControl("chkAIS").Visible = False
        End If
        If (Not e.Item.DataItem("OPERATOR_D") Is DBNull.Value) Then
            e.Item.FindControl("chkDTAC").Visible = True
        Else
            e.Item.FindControl("chkDTAC").Visible = False
        End If
        If (Not e.Item.DataItem("SENT_STATUS_A") Is DBNull.Value) Then
            CType(e.Item.FindControl("lblSentStatus_A"), Label).Text = IIf(e.Item.DataItem("SENT_STATUS_A") = "S", "Sent", "Not Yet")
        End If
        If (Not e.Item.DataItem("SENT_STATUS_D") Is DBNull.Value) Then
            CType(e.Item.FindControl("lblSentStatus_D"), Label).Text = IIf(e.Item.DataItem("SENT_STATUS_D") = "S", "Sent", "Not Yet")
        End If
        'rptCount += 1
